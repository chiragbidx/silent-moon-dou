...
scripts/                    # Ops helpers and unified runtime
  dev-supervisor.js         # Starts Next.js server and git poller
...
pnpm run start:supervised # starts supervisor (next start + git poller)
```

Dev server / supervisor notes
- `pnpm run dev` runs Next.js directly.
- `pnpm run start` runs `next start`.
- `pnpm run start:supervised` runs `scripts/dev-supervisor.js` (Next.js + git poller).
- `pnpm run analyze` runs webpack build analysis and writes reports to `.next/analyze/{client,nodejs,edge}.html`.
- `scripts/dev-supervisor.js` is available when you need a supervised process that starts `next start` and `git-poll`.
...
- Import optimization:
  - `next.config.ts` enables `experimental.optimizePackageImports` for `lucide-react`, `@tanstack/react-table`, and actively used Radix packages.
